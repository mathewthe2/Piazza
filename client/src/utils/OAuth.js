...
export default class OAuth extends Component {
  ...
  fetchData = async() => {
    const response = await fetch('/review/dianping', { credentials : 'same-origin' });
  ...
  openPopup() {
    ...
    const url = `http://localhost:3001/auth/${provider}?socketId=${socket.id}`
  ...
  render() {
    ...
    return (
      <div>
        ...
        <button onClick={()=>this.fetchData()}>get stuff</button>
      </div>
    )
  }
}